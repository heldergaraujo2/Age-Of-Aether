...
bool FAetherClassPresentationDeterminismTest::RunTest(const FString&){FAetherClassPresentationRegistry R;FString E;const TCHAR* C[]={TEXT("archer"),TEXT("warrior"),TEXT("mage"),TEXT("tank"),TEXT("healer")};for(auto X:C){const FString Evolution=FString::Printf(TEXT("%s.01"),X);auto D=Make(X,*Evolution);R.Register(D,E);}TArray<FString>A,B;R.GetIDs(A);R.GetIDs(B);TestEqual(TEXT("count"),A.Num(),5);for(int32 I=0;I<A.Num();++I)TestEqual(TEXT("stable order"),A[I],B[I]);return true;}
...
bool FAetherClassPresentationResetTest::RunTest(const FString&){UGameInstance* GI=NewObject<UGameInstance>(GetTransientPackage()); UAetherClassPresentationSubsystem* S=NewObject<UAetherClassPresentationSubsystem>(GI);S->SetPresentation(Make(TEXT("archer"),TEXT("archer.01")));S->ResetClassPresentation();FAetherClassPresentationBinding B;TestFalse(TEXT("reset"),S->ResolvePresentation(TEXT("archer"),TEXT("archer.01"),B));return true;}
#endif
