...
bool FAetherEquipmentVisualProfileValidationTest::RunTest(const FString& Parameters)
{
    ...
    Profile->VisualProfileID=TEXT("sword_visual"); Profile->ItemDefinitionID=TEXT("item.sword"); Profile->EquipmentSlot=EAetherEquipmentSlot::MainHand; Profile->VisualType=EAetherEquipmentVisualType::StaticMesh;
...
bool FAetherEquipmentVisualProfileSlotTest::RunTest(const FString& Parameters)
{
    ...
    Profile->VisualProfileID=TEXT("helm_visual"); Profile->ItemDefinitionID=TEXT("item.helm"); Profile->EquipmentSlot=EAetherEquipmentSlot::Head; Profile->VisualType=EAetherEquipmentVisualType::SkeletalMesh;
    TestEqual(TEXT("slot is preserved"),Profile->EquipmentSlot,EAetherEquipmentSlot::Head); TestEqual(TEXT("item identity is preserved"),Profile->ItemDefinitionID,FName(TEXT("item.helm"))); return true;
}