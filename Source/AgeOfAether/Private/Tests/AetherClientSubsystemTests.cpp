...
{
    UAetherClientSubsystem* S=NewObject<UAetherClientSubsystem>(NewObject<UGameInstance>(GetTransientPackage())); FString Op=TEXT("Login"); const uint32 ID=S->BeginRequest(Op,10.0,5.0);
...
{
    UAetherClientSubsystem* S=NewObject<UAetherClientSubsystem>(NewObject<UGameInstance>(GetTransientPackage())); FAetherClientIdentitySnapshot I; I.AccountID=TEXT("Account.1"); I.SessionID=TEXT("Session.1");
...
{
    UAetherClientSubsystem* S=NewObject<UAetherClientSubsystem>(NewObject<UGameInstance>(GetTransientPackage())); S->SetConnectionState(EAetherClientConnectionState::Connected); S->SetScreen(EAetherClientScreen::Gameplay);
...
{
    UAetherClientSubsystem* S=NewObject<UAetherClientSubsystem>(NewObject<UGameInstance>(GetTransientPackage())); FAetherClientIdentitySnapshot I; I.SessionID=TEXT("S"); TestFalse(TEXT("session without account rejected"),S->ApplyIdentity(I,1,0));
...
{
    UAetherClientSubsystem* S=NewObject<UAetherClientSubsystem>(NewObject<UGameInstance>(GetTransientPackage())); S->SetConnectionState(EAetherClientConnectionState::Connecting); TestEqual(TEXT("connecting"),S->GetConnectionState(),EAetherClientConnectionState::Connecting);
