...
bool FAetherInteractionRegistry::RegisterQuest(const FAetherInteractionQuestDefinition&D){if(!D.IsValid())return false;auto K=N(D.QuestID);if(Q.Contains(K))return false;auto C=D;C.QuestID=K;Q.Add(K,C);return true;}
...
const FAetherInteractionQuestDefinition* FAetherInteractionRegistry::FindQuest(const FString&ID)const{return Q.Find(N(ID));}
