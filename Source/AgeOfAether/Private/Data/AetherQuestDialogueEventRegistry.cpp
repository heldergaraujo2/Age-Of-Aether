...
bool FAetherQuestDialogueEventRegistry::RegisterQuest(const FAetherDataQuestDefinition& D,FString& E)
...
bool FAetherQuestDialogueEventRegistry::ResolveQuest(const FString& ID,FAetherDataQuestDefinition& O)const{auto*F=Quests.Find(N(ID));if(!F)return false;O=*F;return true;}
...
bool FAetherQuestDialogueEventRegistry::Validate(TArray<FAetherQuestDialogueEventValidationIssue>& Issues,const FAetherContentRegistry* C,const FAetherAssetRegistry*,const FAetherItemRegistry* I,const FAetherWorldActorRegistry* W,const FAetherLootRewardRegistry* L)const
{
 ...
 for(const auto&P:Quests){const auto&D=P.Value; if(W){if(!D.StartNPCID.IsEmpty()&&!W->Contains(D.StartNPCID))AddIssue(Issues,D.DefinitionID,TEXT("MissingNPCReference"),TEXT("StartNPCID is missing."));if(!D.TurnInNPCID.IsEmpty()&&!W->Contains(D.TurnInNPCID))AddIssue(Issues,D.DefinitionID,TEXT("MissingNPCReference"),TEXT("TurnInNPCID is missing."));} for(const auto&Q:D.PrerequisiteQuestIDs)if(!Quests.Contains(N(Q)))AddIssue(Issues,D.DefinitionID,TEXT("MissingQuestReference"),FString::Printf(TEXT("Missing prerequisite quest '%s'."),*Q)); for(const auto&Q:D.NextQuestIDs)if(!Quests.Contains(N(Q)))AddIssue(Issues,D.DefinitionID,TEXT("MissingQuestReference"),FString::Printf(TEXT("Missing next quest '%s'."),*Q)); for(const auto&R:D.RewardIDs)Reward(D.DefinitionID,R); for(const auto&O:D.Objectives){if(O.Type==EAetherDataQuestObjectiveType::Kill||O.Type==EAetherDataQuestObjectiveType::Talk||O.Type==EAetherDataQuestObjectiveType::Interact)Actor(D.DefinitionID,O.TargetID); if(O.Type==EAetherDataQuestObjectiveType::Collect||O.Type==EAetherDataQuestObjectiveType::Craft)Item(D.DefinitionID,O.TargetID); if(O.Type==EAetherDataQuestObjectiveType::WorldEvent&&!WorldEvents.Contains(N(O.EventID)))AddIssue(Issues,D.DefinitionID,TEXT("MissingWorldEventReference"),FString::Printf(TEXT("Missing world event '%s'."),*O.EventID));}}
 for(const auto&P:Dialogues){
 const auto&D=P.Value;
 auto Outcome=[&](const FString&Owner,EAetherDialogueOutcomeType Type,const FString&ID){
  if(Type!=EAetherDialogueOutcomeType::None&&ID.IsEmpty()){AddIssue(Issues,Owner,TEXT("MissingOutcomeID"),TEXT("Outcome requires an OutcomeID."));return;}
  if(Type==EAetherDialogueOutcomeType::AcceptQuest||Type==EAetherDialogueOutcomeType::CompleteQuest){if(!Quests.Contains(N(ID)))AddIssue(Issues,Owner,TEXT("MissingQuestReference"),FString::Printf(TEXT("Missing quest outcome target '%s'."),*ID));}
  else if(Type==EAetherDialogueOutcomeType::StartEvent){if(!WorldEvents.Contains(N(ID)))AddIssue(Issues,Owner,TEXT("MissingWorldEventReference"),FString::Printf(TEXT("Missing event outcome target '%s'."),*ID));}
  else if(Type==EAetherDialogueOutcomeType::GrantReward){Reward(Owner,ID);}
 };
 for(const auto&Node:D.Nodes){
  if(W&&!Node.SpeakerNPCID.IsEmpty()&&!W->Contains(Node.SpeakerNPCID))AddIssue(Issues,D.DefinitionID,TEXT("MissingNPCReference"),FString::Printf(TEXT("Missing NPC '%s'."),*Node.SpeakerNPCID));
  for(int32 Index=0;Index<Node.Outcomes.Num();++Index)Outcome(D.DefinitionID,Node.Outcomes[Index],Node.OutcomeIDs[Index]);
  for(const auto&Choice:Node.Choices)for(int32 Index=0;Index<Choice.Outcomes.Num();++Index)Outcome(D.DefinitionID,Choice.Outcomes[Index],Choice.OutcomeIDs[Index]);
 }
}
