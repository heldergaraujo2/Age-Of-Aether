...
bool FAetherWorldContentRegistry::Validate(TArray<FAetherWorldContentValidationIssue>& O,const FAetherContentRegistry* CR,const FAetherAssetRegistry* AR,const FAetherItemRegistry* IR,const FAetherWorldActorRegistry* WR,const FAetherLootRewardRegistry* LR,const FAetherQuestDialogueEventRegistry* QR,const FAetherRecipeRegistry* RR)const
{
    ...
    for(const auto& Pair:Maps)
    {
        ...
        for(const auto& C:D.StreamingCells){const FString ID=N(C.DefinitionID);if(CellIDs.Contains(ID))Issue(D.DefinitionID,TEXT("DuplicateStreamingCellID"),ID);CellIDs.Add(ID);if(CellCoords.Contains(C.Coordinates))Issue(D.DefinitionID,TEXT("DuplicateStreamingCellCoordinates"),C.DefinitionID);CellCoords.Add(C.Coordinates);if(C.MinBounds.X<D.MinBounds.X||C.MinBounds.Y<D.MinBounds.Y||C.MaxBounds.X>D.MaxBounds.X||C.MaxBounds.Y>D.MaxBounds.Y)Issue(D.DefinitionID,TEXT("StreamingCellOutsideMap"),C.DefinitionID);for(const auto&A:C.AssetIDs)Asset(D.DefinitionID,A);}
